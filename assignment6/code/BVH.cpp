...
Intersection BVHAccel::getIntersection(BVHBuildNode* node,
  ...
  // TODO Traverse the BVH to find intersection
        ...
        // Step 1: Compute inverse direction of the ray
        Vector3f invDir(1.0f / ray.direction.x,
            1.0f / ray.direction.y,
            1.0f / ray.direction.z);

        // Step 2: Determine if ray direction components are negative
        ...
        // Step 3: Check if ray intersects bounding box
        if (!node->bounds.IntersectP(ray, invDir, dirIsNeg))
            return intersect; // empty/default Intersection

        // Step 4: If this is a leaf node (contains an object), test for intersection
        if (node->left == nullptr && node->right == nullptr) {
            assert(node->object != nullptr);
            return node->object->getIntersection(ray); // Call object's own intersection
        }

        // Step 5: Recursively intersect with child nodes
        Intersection leftInter = getIntersection(node->left, ray);
        Intersection rightInter = getIntersection(node->right, ray);

        // Step 6: Return the closer intersection
